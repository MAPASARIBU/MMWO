...
const getOfficePlansPage = async (req, res) => {
    try {
        ...
        const plans = await prisma.officePlan.findMany({
            where,
            include: {
                mill: true,
                station: true
        ...
        res.render('layout', {
            title: 'Office Plans Schedule',
            body: await renderView('admin/office_plans', { plans, mills, stations, user }),
            user: req.session.user,
            path: '/office-plans'
        });
    } catch (error) {
        console.error(error);
        res.status(500).send('Error loading office plans');
    }
};

const createOfficePlan = async (req, res) => {
    try {
        const { mill_id, station_id, name, interval_type, interval_value, next_due_date } = req.body;

        const plan = await prisma.officePlan.create({
            data: {
...
const editOfficePlan = async (req, res) => {
    try {
        ...
        const plan = await prisma.officePlan.update({
            where: { id: planId },
            data: updateData
        });
        res.json(plan);
...
const deleteOfficePlan = async (req, res) => {
    try {
        const planId = parseInt(req.params.id);
        await prisma.officePlan.delete({
            where: { id: planId }
        });
        res.json({ success: true, message: 'Office Plan deleted successfully' });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: error.message });
    }
...
const bulkCreateOfficeWOs = async (req, res) => {
    try {
        const { plan_ids } = req.body;
        const user = req.session.user || req.user;

        if (!plan_ids || !Array.isArray(plan_ids) || plan_ids.length === 0) {
            return res.status(400).json({ error: 'No Office Plans selected' });
        }
        ...
        for (const plan of plans) {
            ...
            const description = `[MANUAL OFFICE] ${plan.name}\nGenerated manually from Office Plan Dashboard Widget.`;
