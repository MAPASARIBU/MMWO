...
const officeScriptHTML = `
    ...
    function openOfficeAddModal() {
        ...
        <% if (user.role === 'ADMIN') { %>
            ...
            if(millEl) filterOfficeStations(millEl.value);
        <% } %>
...
if (!code.includes('officePlanModal')) {
    code = code.replace(/<script>\s*function showMillStations/, officeModalHTML + "\n<script>\n    function showMillStations");
}
if (!code.includes('openOfficeAddModal() {')) {
    code = code.replace('</script>', officeScriptHTML + '\n</script>');
}

fs.writeFileSync('views/admin/master.ejs', code);
console.log('Modal and scripts injected');
